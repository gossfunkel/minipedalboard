...
int main() {
    ...
    if (ma_sound_init_from_file(&engine, "test_tone_2.wav", MA_SOUND_FLAG_NO_DEFAULT_ATTACHMENT, NULL, NULL, &sound) != MA_SUCCESS) {
        fprintf(stderr, "Failed to initialise sound from file!\n");
        return 2;
    }
    ...
    if (p3d_compress_node_init(&engine.nodeGraph, &comp_conf, NULL, &comp_node)) {
        fprintf(stderr, "Failed to initialise compressor node!\n");
        return 3;
    }

    if (ma_node_attach_output_bus(&sound, 0, &comp_node, 0) != MA_SUCCESS) {
        fprintf(stderr, "Failed to attach sound to comp node!\n");
        return 4;
    }

    if (ma_node_attach_output_bus(&comp_node, 0, ma_node_graph_get_endpoint(&engine.nodeGraph), 0) != MA_SUCCESS) {
        fprintf(stderr, "Failed to attach comp node to graph endpoint!\n");
        return 5;
    }
