...
typedef struct {
    ma_node_config nodeConfig;
    p3d_clip_config clip;
} p3d_clip_node_config;
...
p3d_clip_config p3d_clip_config_init(
    ma_uint32 channels, 
    ma_uint32 sampleRate, 
    float drive,
    float bias,
...
ma_result p3d_clip_init(
    const p3d_clip_config *pConfig, 
    const ma_allocation_callbacks *pAllocationCallbacks, 
    p3d_clip *pClip
);
void p3d_clip_uninit(p3d_clip *pClip, const ma_allocation_callbacks *pAllocationCallbacks);
ma_result p3d_clip_process_pcm_frames(
    p3d_clip *pClip,
    void *pFramesOut,
    const void *pFramesIn,
    ma_uint32 frameCount
...
void p3d_clip_node_process_pcm_frames(
    ma_node *pNode,
    const float **ppFramesIn,
    ma_uint32 *pFrameCountIn,
    float **ppFramesOut,
...
static ma_node_vtable p3d_clip_node_vtable = {
    p3d_clip_node_process_pcm_frames,
    NULL,
    1, // input bus
    1, // output bus
    0  // default flags
